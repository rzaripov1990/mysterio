...
- маскирование идёт по имени ключа рекурсивно по всему документу: `data`,
  `errors`, `extensions`, вложенные объекты и массивы;
- `regex` в блоке нет — ответ GraphQL это структурированный JSON, а не
  логовая строка. Вместо него ключ с **объектным или массивным** значением
  заменяется целиком, поэтому поле вида `fullName { ru kz en }` маскируется
  как единое целое (в глобальном `json_keys` для этого нужен `regex`);
...
| Любой ключ в ответе GraphQL (скаляр, объект или массив) | `graphql.json_keys` | GraphQL |
