...
Маскирующий reverse-proxy перед Loki, Elasticsearch и/или GraphQL API:
чувствительные данные маскируются «на лету», прежде чем попадут к клиенту.
Grafana ходит не напрямую в Loki/Elasticsearch, а через mysterio; GraphQL-
клиент — через маршрут `/graphql`.
...
```
Grafana / клиент ──HTTP(S)──▶ mysterio ──HTTP(S)──▶ Loki / Elasticsearch /
                                   │                  GraphQL API (upstream)
                                   └── маскирует чувствительные поля в ответе
```

- **Reverse-proxy, три независимых бэкенда.** `/loki/*` проксируется на
  `LOKI_URL`, `/elastic/*` — на `ELASTIC_URL`, `/graphql` — на `GRAPHQL_URL`.
  Префикс маршрута mysterio (`/loki` или `/elastic`) отрезается; для Loki
  путь к upstream ещё нормализуется (см. ниже «Префиксы `/loki`»). Каждый
  бэкенд включается отдельно (`LOKI_ENABLED`, `ELASTIC_ENABLED`,
  `GRAPHQL_ENABLED`) — хотя бы один должен быть включён, иначе сервис не
  стартует. Заголовки авторизации (`Authorization`, Basic Auth, Cookie)
  **передаются как есть** для всех бэкендов — сервис не хранит и не
  подменяет учётные данные. Токен для GraphQL тоже приходит снаружи, от
  клиента; в конфигурации сервиса его нет.
...
- **Маскирование ответа Elasticsearch.** Применяется **только** к путям,
  ...
  embedded JSON + `regex`.
- **Маскирование ответа GraphQL.** Маршрут `/graphql` работает по отдельному
  блоку правил `graphql:` — он **заменяет** глобальные `json_keys`/`regex`,
  а не дополняет их. Путь запроса подменяется на path из `GRAPHQL_URL`
  (GraphQL — один эндпоинт); метод, тело и заголовки клиента идут как есть.
  **Ответ всегда JSON:** тело, которое не парсится как JSON (HTML-страница
  ошибки от ingress, plain text, пустое тело), заменяется на GraphQL-конверт
  `{"data":null,"errors":[{...}]}` с промаскированной и обрезанной выдержкой
  из ответа upstream; `Content-Type` всегда `application/json`. В отличие от
  логовых маршрутов, ответы со статусом `>= 400` тоже обрабатываются — сам
  код статуса при этом сохраняется.
- **Проверка GraphQL-запроса.** Правила маскирования сравниваются с ключами
  ответа, а алиас (`hackIIN: IIN`) позволил бы клиенту переименовать ключ и
  обойти маску. Поэтому каждый запрос разбирается до отправки в upstream и
  **алиасы запрещены** (алиас, совпадающий с именем поля, `IIN: IIN`,
  допустим). Всё, что проверить нельзя, тоже отклоняется. Отказ — GraphQL-
  ошибка `{"errors":[{"message":…,"extensions":{"code":…}}]}`, upstream не
  вызывается:

  | Код | Статус | Когда |
  | --- | --- | --- |
  | `ALIASES_NOT_ALLOWED` | 400 | алиас поля в любой операции или фрагменте |
  | `GRAPHQL_PARSE_FAILED` | 400 | запрос — невалидный GraphQL (или больше 15000 токенов) |
  | `QUERY_REQUIRED` | 400 | нет текста запроса — например, persisted query только по хешу |
  | `INVALID_REQUEST` | 400 | невалидный JSON, данные после тела, пустой batch, `query` указан дважды (дубль или другой регистр ключа) |
  | `UNSUPPORTED_REQUEST` | 400 / 405 | POST не с `application/json` (multipart, `application/graphql`), тело у GET, метод кроме GET/HEAD/POST/OPTIONS |
  | `REQUEST_TOO_LARGE` | 413 | тело запроса больше 1 МиБ |
  | `WEBSOCKET_NOT_ALLOWED` | 400 | любой `Upgrade`-запрос — подписки не маскируются, поэтому не проксируются |

  Поддерживаются `POST application/json` с `{"query": …}`, batch-массив таких
  объектов (проверяется каждый) и `GET ?query=`. `?query=` проверяется при
  любом методе и в любом регистре; `OPTIONS` (CORS preflight) пропускается.
...
| `GRAPHQL_ENABLED` | нет | `false` | Включает маршрут `/graphql` |
| `GRAPHQL_URL` | да, если `GRAPHQL_ENABLED=true` | — | Полный URL upstream GraphQL-эндпоинта, например `https://api.example/api/graphql`. Path из этого URL используется как есть: всё, что пришло на `/graphql`, уходит туда |
...
| `MAX_RESPONSE_BYTES` | нет | `33554432` (32 MiB) | Ответы больше этого размера проксируются **без** маскирования (общий лимит на все бэкенды) |
...
| `BASE_PATH` | нет | пусто (корень) | Публичный префикс URL **только** в HTML/JS `/test-me` (например `/mysterio`). Процесс всегда слушает `/test-me`; reverse-proxy должен срезать префикс, как для `/loki` и `/elastic` |
...
- Одни и те же правила используются для обоих логовых бэкендов: `json_keys`
  по структуре, `regex` — для строк Loki и строковых полей `_source` в
  Elastic. Маршрут `/graphql` — исключение: он использует отдельный блок
  `graphql:` и **только** его.

### Структура файла

Два механизма — `json_keys` и `regex` — плюс отдельный блок `graphql:`:
...
graphql:                            # применяется ТОЛЬКО на маршруте /graphql
  json_keys:                        # и ТОЛЬКО этот блок (глобальные не в счёт)
    - name: iin
      keys: [IIN, Person.BIIN]      # ключ или путь через точку
      keep_first: 4                 # опционально: оставить первые N символов
      keep_last: 2                  # опционально: оставить последние M символов
      replace: "******"
    - name: balance
      keys: [BALANCE]
      replace_number: 0             # чем заменить число; по умолчанию null
```

### Блок `graphql:`

Правила для маршрута `/graphql`. Ключевое отличие: они **заменяют**
глобальные `json_keys`/`regex`, а не дополняют их — поэтому блок должен быть
самодостаточным и перечислять все ключи, которые нужно скрыть. Ничего не
наследуется.

- правила применяются ко всему документу: `data`, `errors`, `extensions`,
  вложенные объекты и массивы. **Структура и порядок ключей не меняются** —
  заменяются только строки и числа, `null` и `true`/`false` не трогаются;
- `keys` — путь через точку, сравнивается с концом реального пути значения;
  индексы массивов в путь не входят:

  | `keys` | Что маскируется |
  | --- | --- |
  | `FIRSTNAME` | значение `FIRSTNAME` на любой глубине; если это объект или массив — все строки/числа внутри |
  | `FIRSTNAME.RU` | только `RU` внутри `FIRSTNAME`; `KZ`, `EN` не трогаются |
  | `FIRSTNAME.*` | все прямые дочерние поля `FIRSTNAME`; строка `FIRSTNAME` не маскируется |
  | `Person.FIRSTNAME.RU` | то же, но только внутри `Person` |

- при пересечении правил побеждает более конкретное: привязанное ближе к
  значению, затем с бо́льшим числом имён (не `*`), затем более длинное.
  Один и тот же путь в двух правилах — ошибка при старте;
- `keep_first` / `keep_last` — сколько первых / последних символов строки
  оставить вокруг `replace` (`РАВИЛЬ` → `Р***`, `900621300906` →
  `9006******06`). Символы считаются по буквам, кириллица не ломается.
  Строка, в которой нечего скрыть (не длиннее `keep_first + keep_last`),
  заменяется на `replace` целиком. С `{hmac}` не сочетается;
- `replace_number` — чем заменить число: `0`, `-1`, `0.00` или `null`
  (по умолчанию `null`). Число никогда не превращается в строку, чтобы не
  ломать типизированных GraphQL-клиентов. Учтите: `0` не отличить от
  настоящего нуля, а `null` в поле `Float!` строгий клиент примет за ошибку;
- пути, `keep_*` и `replace_number` работают только в блоке `graphql:`;
  в глобальном `json_keys` они — ошибка при старте;
- `{hmac}` и `normalize` работают так же, как в глобальном блоке;
- при `GRAPHQL_ENABLED=true` пустой `graphql.json_keys` — ошибка при старте
  (частая причина — неверный отступ, из-за которого YAML молча теряет блок);
  загруженные ключи пишутся в лог при старте (`graphql masking rules`), а на
  каждый ответ — `graphql response masking changed=...`;
- страница `/test-me` прогоняет только глобальные `json_keys`/`regex`;
  блок `graphql:` она разбирает, но не применяет.
...
| Поле в ответе GraphQL (ключ или путь `FIRSTNAME.RU`), частичная маска | `graphql.json_keys` | GraphQL |
...
- Включается `TEST_ME_ENABLED=true` (по умолчанию выключена). Процесс
  всегда отдаёт страницу на `/test-me`. Если снаружи URL
  `https://host/mysterio/test-me` и ingress делает
  `rewrite-target: /$2` для `/mysterio(/|$)(.*)`, задайте
  `BASE_PATH=/mysterio` — тогда скрипты и `fetch()` ходят на
  `/mysterio/test-me/...`, а до процесса доходит уже `/test-me`.
